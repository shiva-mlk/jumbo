  ...
  return {
    ...
    commerce: {
      inStore: {
        available: true,
        availability: {
          startsOn: '2014-08-26T22:00:00Z',
          endsOn: '9999-12-31T00:00:00Z'
        }
      },
...
describe('parseStore', () => {
    ...
    const store = parseStore(
      makeRawStore({
        location: {
          latitude: 0,
          longitude: 0,
          address: { street: 'Teststraat', city: 'VEGHEL' }
        }
      })
    )
  ...
  it('maps absent coordinates to null', () => {
    const store = parseStore(
      makeRawStore({ location: { address: { street: 'X', city: 'Y' } } })
    )
  ...
  it('fills in every weekday, leaving days without hours empty', () => {
    ...
    expect(store!.openingHours.monday).toEqual({
      opensAt: '08:00+01:00',
  ...
  it('defaults missing facilities and commerce to safe values', () => {
    ...
    expect(store!.commerce.homeDelivery).toEqual({
      available: false,
      startsOn: null,
      endsOn: null
    })
...
describe('parseStores', () => {
  it('parses the envelope into a list of stores', () => {
    const stores = parseStores({
      stores: [makeRawStore(), makeRawStore({ storeId: '3127' })]
    })
  ...
  it('skips unusable records rather than failing the whole dataset', () => {
    const stores = parseStores({
      stores: [makeRawStore(), { storeId: '9999' }, null, 42]
    })
